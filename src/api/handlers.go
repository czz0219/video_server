...
func CreateUser(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	...
	log.Printf("haven add user to dbops")
...
func AddNewVideo(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	...
	log.Printf("AddNewVideo {Author id : %d, name: %s }\n", nvbody.AuthorId, nvbody.Name)
	if err != nil {
...
func DeleteVideo(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	log.Println("begon api-DeleteVideo")
	if !ValidateUser(w, r) {
		log.Println("X-User-Name error")
		return
	}
	vid := p.ByName("vid-id")
	log.Printf("delete video:%s",vid)
	...
	sendNormalResponse(w, "delete ok", 204)
}
...
func ShowComments(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	...
	vid := p.ByName("vid-id")//GET方式传参，只能通过URL 拼接参数，后端用GO 可以 p.ByName
