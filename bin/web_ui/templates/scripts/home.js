		...
		$("#regbtn").text('Loading...')
    		...
    		registerUser(function(res, err) {
    			if (err != null) {
    				...
    				popupErrorMsg('$.ajax #regbtn error');
    				return;
    			}

    			var obj = JSON.parse(res);
    			console.log("#regbtn:"+obj.toString()+",res:"+res)
	...
	$("#uploadform").on('submit', function(e) {
		e.preventDefault()
	  	...
	  	createVideo(vname, function(res, err) {
	  		...
	  		var obj = JSON.parse(res);
	  		console.log("create a video and server return info:"+obj)
...
function selectVideo(vid) {
	var url = 'http://' + window.location.hostname + ':8080/videos/'+ vid
  	var video = $("#curr-video");//第一个子节点是播放区域,src指代视频来源,会发一个GET 资源的HTTP请求
...
function refreshComments(vid) {
	listAllComments(vid, function (res, err) {
		if (err !== null) {
			...
			popupErrorMsg('loading comments error');
			return
		}
...
function registerUser(callback) {
	...
	$.ajax({
		...
		complete: function(xhr, textStatus) {
			console.log("complete-registerUser:"+xhr.responseText+"status:"+xhr.status)
			if (xhr.status >= 400) {
